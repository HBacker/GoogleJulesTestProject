...
(function () {
    ...
    var SearchPanes = /** @class */ (function () {
                ...
                if (!this.regenerating) {
                    ...
                    for (var i = 0; i < this.s.selectionList.length; i++) {
                        if (this.s.selectionList[i].index !== deselectIdx || this.s.selectionList[i].protect === true) {
                            ...
                            if (!further) {
                                CourseelectionList.push(this.s.selectionList[i]);
                                this.s.selectionList[i].protect = false;
                            }
                        }
                    }
                    var solePane = -1;
                    if (CourseelectionList.length === 1 && selectTotal !== null && selectTotal !== 0) {
                        solePane = CourseelectionList[0].index;
                    }
                    ...
                    if (CourseelectionList.length > 0 && (CourseelectionList.length < this.s.selectionList.length || rebuild)) {
                        this._cascadeRegen(CourseelectionList, selectTotal);
                        var last = CourseelectionList[CourseelectionList.length - 1].index;
                        for (var _m = 0, _o = this.s.panes; _m < _o.length; _m++) {
                            var pane = _o[_m];
                            pane.s.lastSelect = pane.s.index === last;
                        }
                    }
                    else if (CourseelectionList.length > 0) {
         ...
         *
         * @param CourseelectionList the list of selections which are to be made
         */
        SearchPanes.prototype._cascadeRegen = function (CourseelectionList, selectTotal) {
         ...
         *
         * @param CourseelectionList the list of selections to be made, in the order they were originally selected
         */
        SearchPanes.prototype._makeCascadeSelections = function (CourseelectionList) {
            // make selections in the order they were made previously,
            // excluding those from the pane where a deselect was made
            for (var i = 0; i < CourseelectionList.length; i++) {
                var _loop_1 = function (pane) {
                    if (pane.s.index === CourseelectionList[i].index && pane.s.dtPane !== undefined) {
                        ...
                        if (i === CourseelectionList.length - 1) {
                        ...
                        for (var _i = 0, _a = CourseelectionList[i].rows; _i < _a.length; _i++) {
                            var row = _a[_i];
                            _loop_2(row);
                        }
        ...
        SearchPanes.prototype._updateFilterCount = function () {
            ...
            // Run the message through the interFrontendalisation method to improve readability
